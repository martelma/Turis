...
public class AvatarUsersEndpoints : IEndpointRouteHandlerBuilder
{
	public static void MapEndpoints(IEndpointRouteBuilder endpoints)
	{
		var avatarsApiGroup = endpoints.MapGroup("/api/users/{id:guid}/avatar");
	...
	private static async Task<IResult> GetAsync(Guid id, IAvatarUserService avatarUserService, HttpContext httpContext)
	{
		var result = await avatarUserService.GetAsync(id);
		var response = httpContext.CreateResponse(result);

		return response;
	}

	private static async Task<IResult> SaveAsync(Guid id, IFormFile file, IAvatarUserService avatarUserService, HttpContext httpContext)
	{
		using var stream = file.OpenReadStream();
		var result = await avatarUserService.SaveAsync(id, stream, file.FileName);
		var response = httpContext.CreateResponse(result);

		return response;
	}

	private static async Task<IResult> DeleteAsync(Guid id, IAvatarUserService avatarUserService, HttpContext httpContext)
	{
		var result = await avatarUserService.DeleteAsync(id);
		var response = httpContext.CreateResponse(result);
