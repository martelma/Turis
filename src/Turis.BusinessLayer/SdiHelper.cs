...
namespace Turis.BusinessLayer;

public class SdiHelper(AppSettings appSettings,
	...
	IContactService contactService)
{
	...
	public long? SendElectronicInvoice(string codiceCliente, byte[] password, string filenameUID, string[] notifyTo = null)
	{
		var auth = new Auth
		{
			Password = password
		...
		if (notifyTo != null)
			send.paramEmail = new SdiWrapper.FatturaElettronicaAttiva.Email() { Address = notifyTo };

		var hub = GetHubFatturaElettronicaAtttiva();
		var response = hub.SendElectronicInvoice(auth, send);

		var msg = string.Empty;
		if (response.ResultCode == SdiWrapper.FatturaElettronicaAttiva.ResultCode.Success)
		{
			msg = $"IdSdi[{response.IdSdi}] Data[{response.DateSdi}] StatoEmail[{response.StatusEmail}]";
			if (response.StatusEmail == SdiWrapper.FatturaElettronicaAttiva.StatusEmail.Error)
			{ }
	...
	{
		return new SolutionDOC_HubSoapClient();
	}

	long? InvioFatturaElettronicaAttiva(string clientCode, byte[] password, string xmlFilePath, string xmlFileName, string[] notifyTo = null)
	{
		var nomeFileZipFatturaPa = GetNomeFileZipFatturaPA(clientCode, password);
		...
		var idSdi = SendElectronicInvoice(clientCode, password, zipFullFileName, notifyTo);
		return idSdi;
	}
	...
	{
		var sdiWrapper = new SdiWrapper.SdiWrapper(appSettings.UrlFatturaElettronicaAttiva);
		var hub = GetHubFatturaElettronicaAtttiva();
		...
		using (var fs = new FileStream(localPathFile, FileMode.Open, FileAccess.Read))
		{
			...
			do
			{
				...
				try
				{
					...
					var sdiWrapper = new SdiWrapper.SdiWrapper(appSettings.UrlFatturaElettronicaAttiva);
					var hub = sdiWrapper.GetHubFatturaElettronicaAtttiva();
